...
void Laborator9::Init() {
  ...
  // Create a simple quad
  {
    ...
    vector<unsigned short> indices = {0, 1, 3, 1, 2, 3};
  ...
  // normal
  {
    ...
    shader->AddShader("Source/Laboratoare/Laborator9/Shaders/VertexShader.glsl",
                      GL_VERTEX_SHADER);
    shader->AddShader(
        "Source/Laboratoare/Laborator9/Shaders/FragmentShader.glsl",
        GL_FRAGMENT_SHADER);
...
void Laborator9::Update(float deltaTimeSeconds) {
  {
    ...
    RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab9"], modelMatrix, true,
                     TextureManager::GetTexture("earth.png"));
  }

  {
    ...
    RenderSimpleMesh(meshes["box"], shaders["ShaderLab9"], modelMatrix, false,
                     TextureManager::GetTexture("crate.jpg"));
  }
...

  {
    ...
    RenderSimpleMesh(meshes["square"], shaders["ShaderLab9"], modelMatrix,
                     false, TextureManager::GetTexture("grass_bilboard.png"));
  }

  {
    ...
    RenderSimpleMesh(meshes["bamboo"], shaders["ShaderLab9"], modelMatrix,
                     false, TextureManager::GetTexture("bamboo.png"));
  }
}

void Laborator9::FrameEnd() { DrawCoordinatSystem(); }

void Laborator9::RenderSimpleMesh(Mesh* mesh, Shader* shader,
...
Texture2D* Laborator9::CreateRandomTexture(unsigned int width,
                                           unsigned int height) {
  GLuint textureID = 0;
  ...
  // TODO: Set the texture parameters (MIN_FILTER, MAG_FILTER and WRAPPING MODE)
...
void Laborator9::OnMouseBtnRelease(int mouseX, int mouseY, int button,
                                   int mods) {
  // add mouse button release event
}

void Laborator9::OnMouseScroll(int mouseX, int mouseY, int offsetX,
                               int offsetY) {}
